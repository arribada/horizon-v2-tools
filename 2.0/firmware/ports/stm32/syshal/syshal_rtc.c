...
int syshal_rtc_get_timestamp(uint32_t * timestamp)
{
    ...
    HAL_RTC_GetTime(&rtc_handle, &current_time, RTC_FORMAT_BIN);
    HAL_RTC_GetDate(&rtc_handle, &current_date, RTC_FORMAT_BIN);

    currTime.tm_year = (uint16_t)(current_date.Year + YEAR_OFFSET + 2000 - 1900);
    currTime.tm_mday = current_date.Date;
    currTime.tm_mon  = current_date.Month - 1;

    currTime.tm_hour = current_time.Hours;
    currTime.tm_min  = current_time.Minutes;
    currTime.tm_sec  = current_time.Seconds;

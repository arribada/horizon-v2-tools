...
struct
{
    GPIO_TypeDef * port;
    uint32_t pin;
} syshal_gpio_port_pin[GPIO_TOTAL_NUMBER];
...
int syshal_gpio_init(uint32_t pin)
{
    ...
    // WARN: We are casting away const here which is safe to do as the underlying code does not modify the pointer
    // Really the STM32 HAL drivers should have this marked as const
    HAL_GPIO_Init(GPIO_Inits[pin].Port, (GPIO_InitTypeDef *) &GPIO_Inits[pin].Init);

    // Copy the GPIO port and pin values into RAM as these are needed by the __RAMFUNC functions below
    syshal_gpio_port_pin[pin].port = GPIO_Inits[pin].Port;
    syshal_gpio_port_pin[pin].pin = GPIO_Inits[pin].Init.Pin;
...
{
    HAL_GPIO_WritePin(syshal_gpio_port_pin[pin].port, syshal_gpio_port_pin[pin].pin, GPIO_PIN_RESET);
}
...
{
    HAL_GPIO_WritePin(syshal_gpio_port_pin[pin].port, syshal_gpio_port_pin[pin].pin, GPIO_PIN_SET);
}
